...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _submit() async {
    ...
    // ✅ Carte d'identité NON obligatoire → on ne bloque plus ici
    ...
    try {
      ...
      if (_selectedImage != null) {
        ...
        final profileRef = FirebaseStorage.instance
            .ref()
            .child("users_data/$uid/profile.$ext");
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Erreur photo de profil : $e"),
      ...
      if (_identityCardImage != null) {
        ...
        final idCardRef = FirebaseStorage.instance
            .ref()
            .child("users_data/$uid/id_card.$ext");

        try {
          if (kIsWeb) {
            final bytes = await _identityCardImage!.readAsBytes();
            await idCardRef.putData(
              bytes,
              SettableMetadata(contentType: 'image/$ext'),
            );
          } else {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Erreur carte d'identité : $e"),
              backgroundColor: Colors.redAccent,
            ),
          );
        }
      ...
      // ✅ Birthdate safe (évite crash si vide)
      final birthText = birthdateController.text.trim();
      final Timestamp? birthTs = birthText.isEmpty
          ? null
          : Timestamp.fromDate(DateFormat('dd/MM/yyyy').parse(birthText));
      ...
      final userModel = UserModel(
        ...
        birthdate: birthTs, // ✅ peut être null si ton modèle l'accepte
        photoUrl: photoUrl,
        identityCardUrl: idCardUrl, // ✅ optionnel
        role: 'passenger',
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: const Text(
              "Cliquez sur le lien dans l'e-mail reçu pour vérifier votre compte.",
                    ...
                    builder: (context, constraints) {
                      ...
                      return FlexibleSpaceBar(
                        ...
                        titlePadding: const EdgeInsets.symmetric(
                            horizontal: 24, vertical: 16), // marge à gauche
                        title: isCollapsed
                            ? const Align(
                        ...
                        children: [
                          ...
                          _buildPasswordField(
                            ...
                            validator: (v) {
                              ...
                              if (!_isPasswordStrong(v))
                                return "8+ caractères, majuscule, chiffre, spécial.";
                              return null;
                            },
                          ),
                          const SizedBox(height: 16),
                          ...
                          TextFormField(
                            ...
                            validator: (value) {
                              ...
                              // Normalisation : enlève espaces et caractères non numériques (on garde les digits)
                              final digits =
                                  v.replaceAll(RegExp(r'[^0-9]'), '');

                              // ✅ Cas autorisés : champ vide OU uniquement l'indicatif (33)
                              if (v.trim().isEmpty) return null;
                              if (digits == '33')
                                return null; // +33 / +33 / +33  => OK (champ "vide")

                              // ✅ Cas valide France : +33 suivi de 9 chiffres => 33 + 9 = 11 digits
                              if (digits.length != 11 ||
                          ...
                          GestureDetector(
                            ...
                            child: AbsorbPointer(
                              child: TextFormField(
                                ...
                                cursorColor: AppColors.gold,
                                decoration:
                                    _inputDecoration("Date de naissance"),
                                validator: (_) => null,
                              ),
                            ),
                          ),
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              const SizedBox(
                                  height: 24), // un peu plus avant l'adresse
                            ],
                          ),
                          const SizedBox(height: 24),
  ...
  Widget _buildIdentityCardPicker() => GestureDetector(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text("Ajouter carte d'identité",
                style: TextStyle(color: AppColors.gold)),
            if (_identityCardImage != null) ...[
              const Spacer(),
              ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: kIsWeb
                    ? Image.network(_identityCardImage!.path,
                        width: 48, height: 48, fit: BoxFit.cover)
                    : Image.file(File(_identityCardImage!.path),
                        width: 48, height: 48, fit: BoxFit.cover),
              ),
            ]
          ],
  ...
  Future<void> _selectBirthDate() async {
    ...
    final picked = await showDatePicker(
      context: context,
      initialDate:
          now.subtract(const Duration(days: 365 * 18)), // par défaut 18 ans
    ...
    if (picked != null) {
      ...
      if (age < 18) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                "L’âge minimum requis est de 18 ans pour rejoindre Ride My Way."),
            backgroundColor: Colors.redAccent,
